...
def evaluate_objective(details: List[Dict], dataset_name: str,
                       output: str,k: List[int]= [1]) -> Dict[str, Any]:
    ...
    for k_ in k:
        idx=[]
        total, correct = 0, 0
        for sample in tqdm(details,desc=f'start evaluating {dataset_name}'):
        ...
        result[dataset_name].append({"acc@%d" % k_: correct / total if total else 0.0,
...
async def evaluate_llm(details: List[Dict], dataset_name: str,
                       endpoint: str,output: str,k: List[int]= [1],) -> Dict[str, Any]:
    extractor = get_extractor(dataset_name)
